...
- 본문은 긴 설명문 대신 `상황 / 대상 / 효과` 3개 섹션으로 분리한다
- `상황`은 자기 조건을 표시한다
  - 예: `내 전열`, `내 후열`, `내 HP 40%↓`
- `대상`은 적/아군/자신 대상 힌트를 표시한다
  - 예: `적 전열`, `적 최저 HP`, `아군 최저 HP`, `자신`
- `효과`는 실제 행동만 짧게 표시한다
  - 예: `공격 ATKx1.2`, `실드 GRDx1.0`, `밀침 후열`
- 배지는 기존 locale 문구를 최대한 재사용하되, 카드 본문에서는 짧게 축약한다
  - 예: `자신 전열` → `내 전열`
- 색상 규칙:
  - 자기 조건: 파란 계열
  - 아군 대상: 하늘색 계열
