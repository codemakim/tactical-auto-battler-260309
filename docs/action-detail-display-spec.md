...
```
┌──────────────────────────────────┐
│  Shield Bash       ⚔ ATKx1.2    │ ← 카드 이름 + 메인 수치 (한 줄)
│ ┌────┐┌────┐┌────┐┌──────┐      │
│ │내전열││적전열││🗡피해││🛡실드│      │ ← 배지 태그 — 섹션 구분 없이 쭉 나열
│ └────┘└────┘└────┘└──────┘      │    색상만으로 조건/대상/효과 구분
└──────────────────────────────────┘
```
...
- **1행 — 카드 이름 + 메인 수치**:
  ...
  - 우측: 메인 수치 (첫 번째 효과의 아이콘 + 배율), 14px, 효과별 색상
    - DAMAGE: `⚔ ATKx1.2` (빨강 #ff4444)
    - SHIELD: `◆ GRDx0.8` (파랑 #4a9eff)
    - HEAL: `+ GRDx0.5` (초록 #44cc44)
    - MOVE: `→ 전열` (노랑 #ffcc00)
    - BUFF/DEBUFF: `▲ ATK↑ 2T` / `▼ ATK↓ 2T` (초록/보라)
...
미니 카드에 마우스를 올리면 **기존 UICardVisual과 동일한 형태**의 확대 카드가 뜬다.
...
│ 효과                │
│ ┌────────┐┌───────┐ │
│ │🗡ATKx1.2││🛡GRDx0.8│ │ ← 효과 배지
│ └────────┘└───────┘ │
└─────────────────────┘
```

- 기존 `UICardVisual`을 **그대로 재사용** (config에 rarity/classRestriction 전달)
- 팝업 위치: 미니 카드 위 또는 옆 (화면 밖으로 나가지 않도록 clamp)
- 팝업 크기: 기존 UICardVisual 기본값 (110×150) 또는 약간 확대 (130×180)
...
- `src/ui/UICardVisual.ts`: Phaser 배지형 카드 컴포넌트 (카드 편집에서 사용 중) — 호버 팝업으로 재사용
...
export class UIActionMiniCard {
  ...
  private tooltip: UICardVisual | null = null;
...
2. **1행 — 이름 + 수치** (y: 4):
   ...
   - 우측 (w - padX, 4): 메인 수치, 14px, `.setOrigin(1, 0)`
     - `getStructuredEffect(action.effects[0])` → `icon + " " + valueText`
...
5. **마우스 오버 (showTooltip=true 시)**:
   ...
   - `pointerover`: `UICardVisual` 생성 (130×180), 미니 카드 위에 표시
     - config: `{ action, condition, rarity, classRestriction, interactive: false }`
     - 위치: 미니 카드 상단 중앙 기준, 화면 밖 clamp 처리
     - depth: 현재 depth + 100
   - `pointerout`: tooltip `.destroy()` 후 null

**배지 tone 확장 (UICardVisual 영향 없음)**:
...
- `UICardVisual` 자체를 수정하지 않는다 (호버 팝업으로 그대로 재사용)
...
| 배지 카드 (팝업용) | `src/ui/UICardVisual.ts` | `UICardVisual`, `UICardVisualConfig` |
...
- 팝업은 기존 UICardVisual과 동일한 형태 (상황/대상/효과 배지 섹션)
- 기존 테스트 전체 통과
- tsc / prettier 통과
