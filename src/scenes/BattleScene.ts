...
export class BattleScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    // 플레이어 측
    this.add
      .rectangle(midX / 2, GAME_HEIGHT / 2, midX - 20, GAME_HEIGHT - 100, 0x16213e, 0.5)
      .setStrokeStyle(1, 0x334466);

    // 적 측
    this.add
      .rectangle(midX + midX / 2, GAME_HEIGHT / 2, midX - 20, GAME_HEIGHT - 100, 0x2e1a1a, 0.5)
      .setStrokeStyle(1, 0x664433);

    // 레이블
    this.add
      .text(midX / 2, 30, 'PLAYER', {
        fontSize: '16px',
        color: '#4a9eff',
    ...
    // 포지션 레이블
    this.add
      .text(midX / 2 - 100, GAME_HEIGHT / 2 - 150, 'FRONT', {
        fontSize: '12px',
        color: '#335577',
        fontFamily: 'monospace',
      })
    ...
    this.add
      .text(midX / 2 + 100, GAME_HEIGHT / 2 - 150, 'BACK', {
        fontSize: '12px',
        color: '#335577',
        fontFamily: 'monospace',
      })
    ...
    // 턴 오더 UI (우측)
    this.add
      .text(GAME_WIDTH - 80, 60, 'TURN ORDER', {
        fontSize: '12px',
        color: '#888888',
        fontFamily: 'monospace',
      })
    ...
    // 히어로 개입 버튼 (하단)
    const heroBtn = this.add
      .text(GAME_WIDTH / 2, GAME_HEIGHT - 40, '[ HERO INTERVENTION ]', {
        fontSize: '18px',
        color: '#ffcc00',
        fontFamily: 'monospace',
      })
  ...
  private createPlaceholderUnits(): void {
    ...
    for (const unit of unitPositions) {
      // 유닛 박스
      this.add.rectangle(unit.x, unit.y, 70, 70, unit.color, 0.3).setStrokeStyle(2, unit.color);

      // 유닛 이름
      this.add
        .text(unit.x, unit.y + 50, unit.label, {
          fontSize: '11px',
          color: '#cccccc',
          fontFamily: 'monospace',
        })
