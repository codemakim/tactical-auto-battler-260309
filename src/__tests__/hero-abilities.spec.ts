...
import { executeQueuedAbility, canIntervene, executeIntervention, heroEditAction } from '../systems/HeroInterventionSystem';
...
describe('HeroDefinitions', () => {
  ...
  it('모든 영웅 정의에 공통 EDIT_ACTION 능력이 포함된다', () => {
    for (const def of Object.values(HERO_DEFINITIONS)) {
      const editAbility = def.abilities.find(
        a => a.abilityType === AbilityType.EDIT_ACTION,
      );
  ...
  it('HEAL 효과: 아군을 회복한다', () => {
    ...
    const damagedState = {
      ...
      units: state.units.map(u =>
        u.id === ally.id ? { ...u, stats: { ...u.stats, hp: u.stats.maxHp - 10 } } : u,
      ),
    };
