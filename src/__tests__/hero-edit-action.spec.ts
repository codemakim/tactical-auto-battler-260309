...
describe('영웅 액션 카드 편집 (공통 능력)', () => {
  ...
  beforeEach(() => {
    ...
    const archer = createUnit(
      createCharacterDef('Sylva', CharacterClass.ARCHER),
      Team.PLAYER,
      Position.BACK,
    );
    const enemy = createUnit(
      createCharacterDef('Shade', CharacterClass.ASSASSIN),
      Team.ENEMY,
      Position.FRONT,
    );
  ...
  it('아군 유닛의 액션 슬롯을 교체할 수 있다', () => {
    ...
    const { state: newState, events } = heroEditAction(
      state, targetId, 2, advanceAction, advanceCondition,
    );
  ...
  it('개입 1회를 소모한다', () => {
    const targetId = state.units.find(u => u.name === 'Aldric')!.id;
    const { state: newState } = heroEditAction(
      state, targetId, 2, advanceAction, advanceCondition,
    );

    expect(newState.hero.interventionsRemaining).toBe(
      state.hero.interventionsRemaining - 1,
    );
  });
  ...
  it('적군 유닛은 편집 불가', () => {
    ...
    const { state: newState, events } = heroEditAction(
      state, enemyId, 0, advanceAction, advanceCondition,
    );
  ...
  it('잘못된 슬롯 인덱스는 편집 불가', () => {
    const targetId = state.units.find(u => u.name === 'Aldric')!.id;
    const { state: newState, events } = heroEditAction(
      state, targetId, 5, advanceAction, advanceCondition,
    );
  ...
  it('사망한 유닛은 편집 불가', () => {
    state = {
      ...
      units: state.units.map(u =>
        u.name === 'Aldric' ? { ...u, isAlive: false } : u,
      ),
    };
    const targetId = state.units.find(u => u.name === 'Aldric')!.id;
    const { state: newState, events } = heroEditAction(
      state, targetId, 0, advanceAction, advanceCondition,
    );
...
describe('전투 종료 후 actionSlots 원복', () => {
  ...
  it('전투 중 편집된 슬롯이 전투 시작 전 상태로 복원된다', () => {
    ...
    // 영웅이 슬롯 2를 편집
    const { state: editedState } = heroEditAction(
      state, state.units[0].id, 2, advanceAction, advanceCondition,
    );
  ...
  it('예비 유닛도 전투 종료 후 원복된다', () => {
    ...
    const reserve = createUnit(
      createCharacterDef('Kael', CharacterClass.LANCER),
      Team.PLAYER,
      Position.BACK,
    );
    const enemy = createUnit(
